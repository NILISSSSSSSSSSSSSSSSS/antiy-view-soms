...
export default class DateRange extends Component {
  ...
  UNSAFE_componentWillReceiveProps (nextProps) {
    if (nextProps.resetKey && this.nextResetKey !== nextProps.resetKey) {
      ...
      const values = nextProps.value || nextProps.initialValue || [null, null]
      //重置初始化日期
  ...
  render () {
    const { format = 'YYYY-MM-DD', style, placeholder = ['开始日期', '结束日期'], itemStyle, allowClear } = this.props
    ...
    return (
      <div className="daterange-custom" style={{ ...style }} key={this.nextResetKey}>
        <DatePicker getCalendarContainer={triggerNode => triggerNode.parentNode} {...obj} value={startValue} format={format} placeholder={placeholder[0]} style={{ ...itemStyle }} disabledDate={(date) => { return this.disabledDate('start', date) }} onChange={(date) => { this.onChange('start', date) }} />
        <span className="split">-</span>
        <DatePicker getCalendarContainer={triggerNode => triggerNode.parentNode} {...obj} value={endValue} format={format} placeholder={placeholder[1]} style={{ ...itemStyle }} disabledDate={(date) => { return this.disabledDate('end', date) }} onChange={(date) => { this.onChange('end', date) }} />
      </div>
    )
  }
}
