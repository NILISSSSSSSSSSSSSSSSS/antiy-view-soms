...
export class OperateLog extends Component {
  constructor (props) {
    super(props)
    this.state = {
      list: {},
  ...
  getList = (page, size, is = false) => {
    ...
    api.operatelogList(postParms).then(response => {
      if (response && response.head && response.head.code === '200') {
        this.setState({
          list: response.body
        })
  ...
  render () {
    ...
    this.defaultFields = [
      { label: '命令', maxLength: 30, key: 'command', span: 12, type: 'input', placeholder: '请输入命令' },
      { label: '设备综合查询', maxLength: 30, key: 'blurQueryField', span: 12, type: 'input', placeholder: '设备IP/编号' }
    ]
    this.filterFields = [
        ...
        <section className="table-wrap table-style">
          <div className="table-btn">
            <div className="left-btn">
              {hasAuth(defendPermission.PORTAL_OPERATE_LOG_EXPORT) ? <Button type="primary" onClick={this.exportAdmittance}>导出</Button> : null}
            </div>
          </div>
          <Table rowKey="optlogId" onChange={this.handleChange} columns={columns} dataSource={list ? list.items : []} pagination={false} />
          {
            list.totalRecords > 0 &&
...
const OperateLogForm = Form.create()(OperateLog)
export default withRouter(connect(mapStateToProps)(OperateLogForm))
