...
class AssetInstalls extends Component {
  constructor (props) {
    super(props)
    this.state = {
      show: false,
  ...
  render () {
    ...
    return (
      <div className="BaseLine">
        <h2 className="page-title">装机模板流程</h2>
        <div className="detail-content">
          <div className="main-content" draggable="true" onMouseDown={this.drags}>
            <img src={require('@/assets/asset_install.svg')} alt=""/>
            <div className="handle">
              {
                userMap.check_template.length > 5 ? (
                  <Tooltip title={userMap.check_template} placement="top">
                    <Button type="primary" icon="user" className="backbtnClass22"
                      onClick={()=>this.showModal('check_template')}>{ userMap.check_template}</Button>
                  </Tooltip>
                ) : (<Button type="primary" icon="user" className="backbtnClass22"
                  onClick={()=>this.showModal('check_template')}>{ userMap.check_template}</Button>)
              }
...
export default connect()(AssetInstalls)
