  ...
  constructor (props) {
    ...
    this.state = {
      ...
      list: [{
        ...
        flowNodeTag: 'asset_ty_zx',
        roleId: ''
      }, {
        flowId: analysisUrl(this.props.location.search).flowId,
        flowNodeTag: 'asset_ty_sp',
        roleId: ''
      }],
      init: undefined,
      text: undefined
  ...
  render () {
    ...
    return (
      <div className="HardwareBack">
        <h2 className="page-title">资产退役流程</h2>
        <div className="detail-content">
          <div className="main-content">
            <img src={require('@/assets/hardware.svg')} alt=""/>
            <div className="handle">
              {
                userMap.asset_ty_sp.length > 5 ? (
                  <Tooltip title={userMap.asset_ty_sp} placement="top">
                    <Button type="primary" icon="user" className="backbtnClass3"
                      onClick={()=>this.showModal('asset_ty_sp')}>{ userMap.asset_ty_sp}</Button>
                  </Tooltip>
                ) : (<Button type="primary" icon="user" className="backbtnClass3"
                  onClick={()=>this.showModal('asset_ty_sp')}>{ userMap.asset_ty_sp}</Button>)
              }
              {
                userMap.asset_ty_zx.length > 5 ? (
                  <Tooltip title={userMap.asset_ty_zx} placement="top">
                    <Button type="primary" icon="user" className="backbtnClass4"
                      onClick={()=>this.showModal('asset_ty_zx')}>{ userMap.asset_ty_zx}</Button>
                  </Tooltip>
                ) : (<Button type="primary" icon="user" className="backbtnClass4"
                  onClick={()=>this.showModal('asset_ty_zx')}>{ userMap.asset_ty_zx}</Button>)
              }
